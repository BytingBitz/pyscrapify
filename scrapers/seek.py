...
class Scraper:
    ...
    class Validators:
        ...
        def validate_data_block(block: list):
            ...
            try:
                if not Scraper.Constants.year_pattern.match((block[Scraper.Constants.data_year_idx].split()[1])):
                    raise SE.UnexpectedData(f'Expected year at second block index:\n{block}')
                if not block[Scraper.Constants.data_challenge_idx] == Scraper.Constants.challenge_text:
    ...
    class Parsers:
        ...
        def extract_data_indices(texts: list[list]) -> list[int]:
            ...
            return [i for i, x in enumerate(texts) if x == Scraper.Constants.good_text]
        @staticmethod
        def extract_data_bounds(idx: int) -> dict[str, int]:
            ''' Returns: Dict of start and end indexs for relevant review data in list. '''
            start_idx = idx - Scraper.Constants.data_start_offset
            end_idx = idx + Scraper.Constants.data_length - Scraper.Constants.data_start_offset
