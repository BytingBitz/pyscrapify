...
def review_volume(soup, website, reviews_per_page):
    ...
    try:
        if website == "Indeed":
            ...
            try: 
                number_reviews = int(overview_data.find('span').find('b').text.replace(',', ''))
            except: 
                number_reviews = int(re.findall(r'\d+',overview_data.find('span').text)[0])
        elif website == "Seek":
...
def indeed_scrape(organisation, indeed_url, output_name, indeed_country):
    ...
    for page in range(number_pages):
        ...
        if page == 0: 
            ...
            soup, jump = grab_HTML("Indeed", indeed_url, page * reviews_per_page, indeed_country), 1
